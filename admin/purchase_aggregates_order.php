...
<head>
    ...
    <title>Pending P.O. - Purchase Order Aggregates</title>
...
<?php
	...
	if(isset($_GET['search_table'])){
		...
		$sql = "SELECT * FROM purchase_order_aggregates p ".$string." ".$string_date." ".$string_ext." AND quantity != 0";

		$sql_result = mysqli_query($db, $sql); 
		$total = mysqli_num_rows($sql_result);

		$adjacents = 3;
		$targetpage = "purchase_aggregates_order.php"; //your file name
		$page = $_GET['page'];
	    ...
	    $query = "SELECT p.purchase_order_aggregates_no, p.item_no, p.quantity, p.price, l.truck, l.unit, p.supplier_name, DATE_FORMAT(p.date_po_aggregates,'%m/%d/%y') as date_po_aggregates1, p.remarks, p.office, p.received
					FROM purchase_order_aggregates p, item_list l
					".$string." ".$string_date." ".$string_ext."
					AND p.item_no = l.item_no 
					AND p.remarks = 'Pending'
					ORDER BY purchase_order_aggregates_no DESC LIMIT $start, $limit";
...
?>
					<td style="background: green; color: white;"><strong><?php echo $row['remarks']; ?></strong></td>
<?php
...
?>
					<td style="background: orange; color: white;"><strong><?php echo $row['remarks']; ?></strong></td>
<?php
				}
?>
				</tr>
