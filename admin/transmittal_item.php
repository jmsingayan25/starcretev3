...
<?php
    ...
    if(isset($_REQUEST['pass_transmittal_no']) && isset($_REQUEST['pass_from_office']) && isset($_REQUEST['pass_id'])){
    	$_SESSION['transmittal_no'] = $_REQUEST['pass_transmittal_no'];
    	$_SESSION['from_office'] = $_REQUEST['pass_from_office'];
    	$_SESSION['id'] = $_REQUEST['pass_id'];
    }

    $pass_transmittal_no = $_SESSION['transmittal_no'];
    $pass_from_office = $_SESSION['from_office'];
                ...
                <div class="row">
                    <div class="col-md-12 page_links">
                        ...
                        <ol class="breadcrumb">
                            ...
                            <li><i class="fa fa-address-book"></i><a href="transmittal_item.php" style="color: blue;">Items</a></li>			  	
                        </ol>
                    </div>
                </div>
                <div class="row">
	                	...
	                	<section class="panel">
                            <header class="panel-heading">
                                <div class="row">
                                	...
                                	<div class="col-md-6">
                                		<span><strong>Origin: <?php echo ucfirst($pass_from_office); ?></strong></span>
                                	</div>
                                </div>
                            </header>
...
<?php
	...
	$sql = "SELECT * FROM transmittal 
			...
			AND from_office = '$pass_from_office' 
			AND office = '$office'
...
?>
					<form action="transmittal_item.php" method="post" onsubmit="return confirm('Confirm <?php echo $row['item_no']; ?> received?')">
						<button type="submit" id="submit" name="submit" value="<?php echo $row['transmittal_id']; ?>" class="btn btn-success btn-sm"><strong>Received</strong></button>
					</form>		
<?php
				}else{
?>
...
<?php
	...
	if(isset($_POST['submit'])){
		...
		$from_office = ucfirst($row['from_office']);
		...
		$history = "INSERT INTO history(table_report, transaction_type, item_no, detail, history_date, office)
					VALUES('Transmittal','Received Item','$item_no','$delivered_by from $from_office transferred $quantity of $item_no to Head Office','$datetime','$office')";
		// echo $history;
		// echo $sql;
		if(mysqli_query($db, $sql) && mysqli_query($db, $history)){
			phpAlert('Confirmed received item');
